/* AUTO-GENERATED by scripts/generate-from-registry.mts — DO NOT EDIT. Regenerate with: npm run generate */
import type { GeneratedResource } from './types';

export const tasksResource: GeneratedResource = {
	resource: 'tasksAdvanced',
	displayName: 'Tasks Advanced',
	operations: [
		{
			value: 'addCardDependency',
			name: 'Add Card Dependency',
			action: 'Mark a card as blocked by another card of the same project (Linear-like dependency)',
			description: 'Mark a card as blocked by another card of the same project (Linear-like dependency). Rejects cycles and cross-project links.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/dependencies","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Blocker Card ID',
					name: 'blockerCardId',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'aiDecomposeTask',
			name: 'AI Decompose Task',
			action: 'Break one task down into concrete subtasks with AI',
			description: 'Break one task down into concrete subtasks with AI. Send the task WORDING, never a card ID: this tool reads nothing from the board and writes nothing to it — it returns a proposed list of subtasks that you create afterwards with create_card (parent_card_id = the parent) once the person agrees. To decompose a card that already exists, read it with get_card first and pass its title and description here.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/ai/decompose","queryParams":[]},
			properties: [
				{
					displayName: 'Title',
					name: 'title',
					type: 'string',
					required: true,
					description: 'The task to break down, worded the way the person said it ("refaire la page tarifs", "lancer la campagne de janvier"). This is what the model works from — an ID, a slug or a uuid here produces meaningless subtasks.',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Context',
							name: 'context',
							type: 'string',
							description: 'Surrounding context the model cannot guess: the project, the deadline, who does the work, constraints already discussed in the conversation',
							default: '',
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							description: 'What the task involves, if any detail was given. The more concrete this is, the less generic the subtasks come back.',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'aiGenerateFromModule',
			name: 'AI Generate From Module',
			action: 'Same as ai_generate_tasks_from_module: turn a completed module deliverable into a list of PROPOSED tasks',
			description: 'Same as ai_generate_tasks_from_module: turn a completed module deliverable into a list of PROPOSED tasks. Read the module with list_completed_modules, pass its name and its content. Nothing is written to a board — create the cards with create_card once the person agrees.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/ai/generate-from-module","queryParams":[]},
			properties: [
				{
					displayName: 'Module Name',
					name: 'moduleName',
					type: 'string',
					required: true,
					description: 'Human name of the module the actions come from, e.g. "Étude de marché", "Business Model Canvas". It goes into the prompt, so send the label the person recognises, not a slug or a uuid.',
					default: '',
				},
				{
					displayName: 'Module Data',
					name: 'moduleData',
					type: 'json',
					required: true,
					description: 'The module deliverable itself, as the structured object list_completed_modules returns. Send it whole — the model reads its fields to find the actions to do; a summary or an empty object gives generic tasks. (provide a JSON object)',
					default: '{}',
				}
			],
		},
		{
			value: 'aiGenerateTasksFromModule',
			name: 'AI Generate Tasks From Module',
			action: 'Turn the deliverable of a completed module into a list of PROPOSED tasks',
			description: 'Turn the deliverable of a completed module into a list of PROPOSED tasks. Read the module first with list_completed_modules, then pass its name and its content here. Nothing is written to a board: the answer is a proposal you create with create_card once the person agrees.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/ai/generate-from-module","queryParams":[]},
			properties: [
				{
					displayName: 'Module Name',
					name: 'moduleName',
					type: 'string',
					required: true,
					description: 'Human name of the module the actions come from, e.g. "Étude de marché", "Business Model Canvas". It goes into the prompt, so send the label the person recognises, not a slug or a uuid.',
					default: '',
				},
				{
					displayName: 'Module Data',
					name: 'moduleData',
					type: 'json',
					required: true,
					description: 'The module deliverable itself, as the structured object list_completed_modules returns. Send it whole — the model reads its fields to find the actions to do; a summary or an empty object gives generic tasks. (provide a JSON object)',
					default: '{}',
				}
			],
		},
		{
			value: 'archiveSprint',
			name: 'Archive Sprint',
			action: 'Archive a milestone (« jalon »)',
			description: 'Archive a milestone (« jalon »)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/sprints/{sprint_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Sprint ID',
					name: 'sprint_id',
					type: 'string',
					required: true,
					description: 'The sprint ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'bulkCardAction',
			name: 'Bulk Card Action',
			action: 'Apply ONE action to up to 100 kanban cards at once: `delete` (definitive, no bin), `move_column` (to `columnId`, appended at the bottom of that column — the column decides the new status), or `assign` (the assignee list of every card becomes `[assigneeId]`, or EMPTY when `assigneeId` is null — it replaces, it does not add)',
			description: 'Apply ONE action to up to 100 kanban cards at once: `delete` (definitive, no bin), `move_column` (to `columnId`, appended at the bottom of that column — the column decides the new status), or `assign` (the assignee list of every card becomes `[assigneeId]`, or EMPTY when `assigneeId` is null — it replaces, it does not add). Access is checked card by card and the call is NOT atomic: the result is `{ results: [{ID, ok, error?}], processed }` — always report the failures, some cards may have changed and others not. For changing fields (priority, dates, tags…) on many cards use `bulk_update_cards` instead. Get IDs from `list_cards`, column IDs from `get_board_schema`, user IDs from `list_collaborators`. Name the count and the action to the person before calling.',
			routeSpec: {"method":"POST","path":"/api/tasks/cards/bulk","queryParams":[]},
			properties: [
				{
					displayName: 'Action',
					name: 'action',
					type: 'options',
					required: true,
					description: 'The single action applied to every ID',
					default: 'assign',
					options: [
						{ name: 'Assign', value: 'assign' },
						{ name: 'Delete', value: 'delete' },
						{ name: 'Move Column', value: 'move_column' },
					],
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'Card IDs (UUID). (provide a JSON array).',
					default: '[]',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Assignee ID',
							name: 'assigneeId',
							type: 'string',
							description: 'Assign only: the single assignee. null clears assignees.',
							default: '',
						},
						{
							displayName: 'Column ID',
							name: 'columnId',
							type: 'string',
							description: 'Move_column only: target column ID',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'bulkUpdateCards',
			name: 'Bulk Update Cards',
			action: 'Apply the same patch to many cards at once (max 200)',
			description: 'Apply the same patch to many cards at once (max 200). Same fields as update_card.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/bulk-update","queryParams":[]},
			properties: [
				{
					displayName: 'Card IDs',
					name: 'card_ids',
					type: 'json',
					required: true,
					description: 'Card UUIDs (1-200). (provide a JSON array).',
					default: '[]',
				},
				{
					displayName: 'Patch',
					name: 'patch',
					type: 'json',
					required: true,
					description: 'Fields to set on every card. (provide a JSON object).',
					default: '{}',
				}
			],
		},
		{
			value: 'changeTaskPropertyType',
			name: 'Change Task Property Type',
			action: 'Convert a custom task property to another TYPE (text → number, select → multi_select, date → datetime…) and convert every existing cell with it',
			description: 'Convert a custom task property to another TYPE (text → number, select → multi_select, date → datetime…) and convert every existing cell with it. ALWAYS call it first with `dryRun: true`: that returns `{ converted, lost, sample }` without writing — `lost` is the number of cells whose value cannot survive the conversion (a text that is not a number, a choice that has no equivalent) and `sample` shows some of them. Show that preview to the person; only then call again with `dryRun: false`, which writes and is not reversible (lost values are gone). This is the « appel dédié » that `update_task_property` refers to for the type. Types: text, long_text, number, date, datetime, checkbox, URL, email, phone, select, multi_select, user, multi_user.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/property-definitions/{id}/type","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'Property definition ID (from list_task_properties)',
					default: '',
				},
				{
					displayName: 'To Type',
					name: 'toType',
					type: 'options',
					required: true,
					description: 'Target type',
					default: 'checkbox',
					options: [
						{ name: 'Checkbox', value: 'checkbox' },
						{ name: 'Date', value: 'date' },
						{ name: 'Datetime', value: 'datetime' },
						{ name: 'Email', value: 'email' },
						{ name: 'Long Text', value: 'long_text' },
						{ name: 'Multi Select', value: 'multi_select' },
						{ name: 'Multi User', value: 'multi_user' },
						{ name: 'Number', value: 'number' },
						{ name: 'Phone', value: 'phone' },
						{ name: 'Select', value: 'select' },
						{ name: 'Text', value: 'text' },
						{ name: 'URL', value: 'url' },
						{ name: 'User', value: 'user' },
					],
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Dry Run',
							name: 'dryRun',
							type: 'boolean',
							description: 'Whether true = preview only (converted / lost / sample), nothing written. Default false.',
							default: false,
						},
					],
				}
			],
		},
		{
			value: 'createBoard',
			name: 'Create Board',
			action: 'Create a kanban board',
			description: 'Create a kanban board',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards","queryParams":[]},
			properties: [
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Project ID',
							name: 'projectId',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createCardChecklist',
			name: 'Create Card Checklist',
			action: 'Create a checklist on a card',
			description: 'Create a checklist on a card',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/checklists","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Title',
					name: 'title',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'createCardComment',
			name: 'Create Card Comment',
			action: 'Add a comment to a card',
			description: 'Add a comment to a card',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/comments","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Content',
					name: 'content',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'createCardSubtask',
			name: 'Create Card Subtask',
			action: 'Add a subtask to a card',
			description: 'Add a subtask to a card',
			routeSpec: {"method":"POST","path":"/api/tasks/cards/{card_id}/subtasks","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Title',
					name: 'title',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Assigned To',
							name: 'assigned_to',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Due Date',
							name: 'due_date',
							type: 'string',
							description: 'ISO 8601 datetime',
							default: '',
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimated_minutes',
							type: 'number',
							description: 'Estimate in minutes (1-43200)',
							default: 0,
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createCardTemplate',
			name: 'Create Card Template',
			action: 'Crée un modèle de tâche (portée projet)',
			description: 'Crée un modèle de tâche (portée projet). Un champ omis ne pose rien à l\'instanciation (instantiate_card_template) — jamais écrit en vide sur la carte créée.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/task-templates","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					description: 'Nom affiché dans le picker « Nouvelle tâche » — DISTINCT du titre de la tâche créée',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Assigned To',
							name: 'assignedTo',
							type: 'json',
							description: 'User UUIDs. Absent/empty = the template assigns no one. (provide a JSON array)',
							default: '[]',
						},
						{
							displayName: 'Checklist Items',
							name: 'checklistItems',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Custom Properties',
							name: 'customProperties',
							type: 'json',
							description: 'Provide a JSON object',
							default: '{}',
						},
						{
							displayName: 'Description Md',
							name: 'descriptionMd',
							type: 'string',
							description: 'Markdown brut',
							default: '',
						},
						{
							displayName: 'Due Offset Days',
							name: 'dueOffsetDays',
							type: 'number',
							description: 'Signed day offset from the instantiation date (e.g. -2 = two days before a milestone). Resolved to an absolute date only when instantiate_card_template runs — never store an absolute date on a template.',
							default: 0,
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimatedMinutes',
							type: 'number',
							description: 'Estimate in minutes (1-43200). Absent = the template poses no estimate.',
							default: 0,
						},
						{
							displayName: 'Is Code Task',
							name: 'isCodeTask',
							type: 'boolean',
							description: 'Whether to enable is code task',
							default: false,
						},
						{
							displayName: 'Is Important',
							name: 'isImportant',
							type: 'boolean',
							description: 'Whether to enable is important',
							default: false,
						},
						{
							displayName: 'Is Milestone',
							name: 'isMilestone',
							type: 'boolean',
							description: 'Whether to enable is milestone',
							default: false,
						},
						{
							displayName: 'Is Urgent',
							name: 'isUrgent',
							type: 'boolean',
							description: 'Whether to enable is urgent',
							default: false,
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
						{
							displayName: 'Start Offset Days',
							name: 'startOffsetDays',
							type: 'number',
							description: 'Signed day offset from the instantiation date (e.g. -2 = two days before a milestone). Resolved to an absolute date only when instantiate_card_template runs — never store an absolute date on a template.',
							default: 0,
						},
						{
							displayName: 'Status Category',
							name: 'statusCategory',
							type: 'options',
							default: 'backlog',
							options: [
								{ name: 'Backlog', value: 'backlog' },
								{ name: 'Blocked', value: 'blocked' },
								{ name: 'Done', value: 'done' },
								{ name: 'In Progress', value: 'in_progress' },
								{ name: 'Todo', value: 'todo' },
							],
						},
						{
							displayName: 'Tag IDs',
							name: 'tagIds',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							description: 'Titre de la tâche créée à l’instanciation',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createChecklistItem',
			name: 'Create Checklist Item',
			action: 'Add a checklist item',
			description: 'Add a checklist item',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/checklists","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Title',
					name: 'title',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'createColumn',
			name: 'Create Column',
			action: 'Create a column',
			description: 'Create a column',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/{board_id}/columns","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'createProjectTaskTag',
			name: 'Create Project Task Tag',
			action: 'Create a task tag on a project',
			description: 'Create a task tag on a project',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/tags","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							description: 'Hex color, e.g. #ff6900',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createRecurringTask',
			name: 'Create Recurring Task',
			action: 'Create a recurring task on a board (daily/weekly/monthly…)',
			description: 'Create a recurring task on a board (daily/weekly/monthly…). next_due_date = first occurrence (ISO datetime).',
			routeSpec: {"method":"POST","path":"/api/tasks/boards/{board_id}/recurring","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Title',
					name: 'title',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Frequency',
					name: 'frequency',
					type: 'options',
					required: true,
					default: 'bi-weekly',
					options: [
						{ name: 'Bi Weekly', value: 'bi-weekly' },
						{ name: 'Daily', value: 'daily' },
						{ name: 'Every N Days', value: 'every_n_days' },
						{ name: 'Every N Weeks', value: 'every_n_weeks' },
						{ name: 'Monthly', value: 'monthly' },
						{ name: 'Weekly', value: 'weekly' },
						{ name: 'Yearly', value: 'yearly' },
					],
				},
				{
					displayName: 'Next Due Date',
					name: 'next_due_date',
					type: 'string',
					required: true,
					description: 'ISO 8601 datetime of the first occurrence',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Column ID',
							name: 'column_id',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Day Of Month',
							name: 'day_of_month',
							type: 'number',
							description: '1-31 (monthly)',
							default: 0,
						},
						{
							displayName: 'Day Of Week',
							name: 'day_of_week',
							type: 'number',
							description: '0-6 (weekly)',
							default: 0,
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
						{
							displayName: 'End Date',
							name: 'end_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimated_minutes',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Month Of Year',
							name: 'month_of_year',
							type: 'number',
							description: '1-12 (yearly)',
							default: 0,
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
						{
							displayName: 'Start Date',
							name: 'start_date',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createSprint',
			name: 'Create Sprint',
			action: 'Create a milestone (« jalon ») on a board',
			description: 'Create a milestone (« jalon ») on a board. The product calls these MILESTONES (« jalons » in French) since 2026-08-08 — say « milestone/jalon » to the user, never « sprint ». The table and the `sprint_ids` field keep their historical names.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/sprints","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							default: '',
						},
						{
							displayName: 'End Date',
							name: 'end_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Goal',
							name: 'goal',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Start Date',
							name: 'start_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Status',
							name: 'status',
							type: 'options',
							default: 'active',
							options: [
								{ name: 'Active', value: 'active' },
								{ name: 'Archived', value: 'archived' },
								{ name: 'Completed', value: 'completed' },
								{ name: 'Upcoming', value: 'upcoming' },
							],
						},
					],
				}
			],
		},
		{
			value: 'createTaskBoardComment',
			name: 'Create Task Board Comment',
			action: 'Add a comment to a task project (board)',
			description: 'Add a comment to a task project (board)',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/{board_id}/comments","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Content',
					name: 'content',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'createTaskBoardLink',
			name: 'Create Task Board Link',
			action: 'Attach a resource to a task project (board)',
			description: 'Attach a resource to a task project (board). link_type = resource|drive_file|external_url|note|meeting ; provide link_id (internal resource) or URL (external).',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/{board_id}/links","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Link Type',
					name: 'link_type',
					type: 'options',
					required: true,
					default: 'drive_file',
					options: [
						{ name: 'Drive File', value: 'drive_file' },
						{ name: 'External URL', value: 'external_url' },
						{ name: 'Meeting', value: 'meeting' },
						{ name: 'Note', value: 'note' },
						{ name: 'Resource', value: 'resource' },
					],
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Label',
							name: 'label',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Link ID',
							name: 'link_id',
							type: 'string',
							default: '',
						},
						{
							displayName: 'URL',
							name: 'url',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createTaskBoardMeeting',
			name: 'Create Task Board Meeting',
			action: 'Create a meeting linked to a task project (title + start_at + end_at) OR link an existing calendar event (event_id alone)',
			description: 'Create a meeting linked to a task project (title + start_at + end_at) OR link an existing calendar event (event_id alone)',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/{board_id}/meetings","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
						{
							displayName: 'End At',
							name: 'end_at',
							type: 'string',
							description: 'ISO 8601 datetime with offset (after start_at)',
							default: '',
						},
						{
							displayName: 'Event ID',
							name: 'event_id',
							type: 'string',
							description: 'Existing calendar event UUID to link (alone)',
							default: '',
						},
						{
							displayName: 'Location',
							name: 'location',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Start At',
							name: 'start_at',
							type: 'string',
							description: 'ISO 8601 datetime with offset',
							default: '',
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Video Conference URL',
							name: 'video_conference_url',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'createTaskDomain',
			name: 'Create Task Domain',
			action: 'Crée un domaine d\'activité pour organiser les projets/boards du portefeuille (ex',
			description: 'Crée un domaine d\'activité pour organiser les projets/boards du portefeuille (ex. « Marketing », « Produit »). color = clé de palette (slate/brown/orange/amber/emerald/sky/indigo/violet/pink/rose), jamais un hex. Posé en fin de liste.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/domains","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Color',
					name: 'color',
					type: 'color',
					required: true,
					description: 'Clé de palette — cf. list_task_domains.',
					default: '',
				}
			],
		},
		{
			value: 'createTaskPriority',
			name: 'Create Task Priority',
			action: 'Crée un niveau de priorité',
			description: 'Crée un niveau de priorité. rank absent = posé au-dessus de tout (le plus urgent). color = clé de palette (slate/brown/orange/amber/emerald/sky/indigo/violet/pink/rose).',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/priorities","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Label',
					name: 'label',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							default: '',
						},
						{
							displayName: 'Is Default',
							name: 'isDefault',
							type: 'boolean',
							description: 'Whether to enable is default',
							default: false,
						},
						{
							displayName: 'Rank',
							name: 'rank',
							type: 'number',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'createTaskProperty',
			name: 'Create Task Property',
			action: 'Crée une propriété custom de tâches',
			description: 'Crée une propriété custom de tâches. boardId absent = propriété visible sur toutes les tâches du projet. Types : text, long_text, number, date, datetime, checkbox, URL, email, phone, select, multi_select, user, multi_user.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/property-definitions","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Label',
					name: 'label',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Type',
					name: 'type',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Board ID',
							name: 'boardId',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Options',
							name: 'options',
							type: 'json',
							description: 'Provide a JSON object',
							default: '{}',
						},
					],
				}
			],
		},
		{
			value: 'createTaskStatus',
			name: 'Create Task Status',
			action: 'Crée un statut de tâche',
			description: 'Crée un statut de tâche. category ∈ backlog/todo/in_progress/blocked/done (catégorie dénormalisée lue par le dashboard, le Gantt, le calendrier et les widgets). color = clé de palette (slate/brown/orange/amber/emerald/sky/indigo/violet/pink/rose), jamais un hex. rank absent = posé en FIN de flux (le plus avancé) — inverse de create_task_priority, où rank absent pose en TÊTE.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/statuses","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Label',
					name: 'label',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Category',
					name: 'category',
					type: 'options',
					required: true,
					default: 'backlog',
					options: [
						{ name: 'Backlog', value: 'backlog' },
						{ name: 'Blocked', value: 'blocked' },
						{ name: 'Done', value: 'done' },
						{ name: 'In Progress', value: 'in_progress' },
						{ name: 'Todo', value: 'todo' },
					],
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							description: 'Clé de palette — cf. list_task_statuses (slate/brown/orange/amber/emerald/sky/indigo/violet/pink/rose). Jamais un hex.',
							default: '',
						},
						{
							displayName: 'Is Default',
							name: 'isDefault',
							type: 'boolean',
							description: 'Whether to enable is default',
							default: false,
						},
						{
							displayName: 'Rank',
							name: 'rank',
							type: 'number',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'createTaskTemplate',
			name: 'Create Task Template',
			action: 'Create a kanban task template',
			description: 'Create a kanban task template. projectId is required (auto-injected from session). columns must have at least 1 entry, each with { name, color?, is_terminal? }.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/templates","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Name',
					name: 'name',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Columns',
					name: 'columns',
					type: 'json',
					required: true,
					description: 'Provide a JSON array',
					default: '[]',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Category',
							name: 'category',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'deleteBoard',
			name: 'Delete Board',
			action: 'Delete a board',
			description: 'Delete a board',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/boards/{board_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteCardAttachment',
			name: 'Delete Card Attachment',
			action: 'Delete an attachment',
			description: 'Delete an attachment',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/attachments/{attachment_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Attachment ID',
					name: 'attachment_id',
					type: 'string',
					required: true,
					description: 'The attachment ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteCardComment',
			name: 'Delete Card Comment',
			action: 'Delete a comment',
			description: 'Delete a comment',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/comments/{comment_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Comment ID',
					name: 'comment_id',
					type: 'string',
					required: true,
					description: 'The comment ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteCardSubtask',
			name: 'Delete Card Subtask',
			action: 'Delete a subtask',
			description: 'Delete a subtask',
			routeSpec: {"method":"DELETE","path":"/api/tasks/subtasks/{subtask_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Subtask ID',
					name: 'subtask_id',
					type: 'string',
					required: true,
					description: 'The subtask ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteCardTemplate',
			name: 'Delete Card Template',
			action: 'Supprime un modèle de tâche (réversible 30 jours)',
			description: 'Supprime un modèle de tâche (réversible 30 jours)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/task-templates/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteChecklistItem',
			name: 'Delete Checklist Item',
			action: 'Delete a checklist item',
			description: 'Delete a checklist item',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/checklists/{item_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Item ID',
					name: 'item_id',
					type: 'string',
					required: true,
					description: 'The item ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteColumn',
			name: 'Delete Column',
			action: 'Delete a column',
			description: 'Delete a column',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/columns/{column_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Column ID',
					name: 'column_id',
					type: 'string',
					required: true,
					description: 'The column ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteProjectTaskTag',
			name: 'Delete Project Task Tag',
			action: 'Delete a project task tag',
			description: 'Delete a project task tag',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/tags/{tag_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Tag ID',
					name: 'tag_id',
					type: 'string',
					required: true,
					description: 'The tag ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteRecurringTask',
			name: 'Delete Recurring Task',
			action: 'Delete a recurring task definition (already-spawned cards are kept)',
			description: 'Delete a recurring task definition (already-spawned cards are kept)',
			routeSpec: {"method":"DELETE","path":"/api/tasks/recurring/{recurring_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Recurring ID',
					name: 'recurring_id',
					type: 'string',
					required: true,
					description: 'The recurring ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteSprint',
			name: 'Delete Sprint',
			action: 'Delete a milestone (« jalon »)',
			description: 'Delete a milestone (« jalon »)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/sprints/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskBoardComment',
			name: 'Delete Task Board Comment',
			action: 'Delete a comment from a task project (board)',
			description: 'Delete a comment from a task project (board)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/boards/{board_id}/comments/{comment_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Comment ID',
					name: 'comment_id',
					type: 'string',
					required: true,
					description: 'The comment ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskBoardLink',
			name: 'Delete Task Board Link',
			action: 'Remove a resource link from a task project (board)',
			description: 'Remove a resource link from a task project (board)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/boards/{board_id}/links/{link_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Link ID',
					name: 'link_id',
					type: 'string',
					required: true,
					description: 'The link ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskBoardMeeting',
			name: 'Delete Task Board Meeting',
			action: 'Unlink a meeting from a task project (the calendar event itself is kept in /calendrier)',
			description: 'Unlink a meeting from a task project (the calendar event itself is kept in /calendrier)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/boards/{board_id}/meetings/{event_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Event ID',
					name: 'event_id',
					type: 'string',
					required: true,
					description: 'The event ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskDomain',
			name: 'Delete Task Domain',
			action: 'Supprime un domaine d\'activité — bascule d\'abord tous ses boards vers « Non classé », puis supprime la ligne',
			description: 'Supprime un domaine d\'activité — bascule d\'abord tous ses boards vers « Non classé », puis supprime la ligne. Refusé si un board n\'a pas pu être rebasculé, ou sur « Non classé » lui-même.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/domains/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskPriority',
			name: 'Delete Task Priority',
			action: 'Supprime un niveau de priorité (réversible)',
			description: 'Supprime un niveau de priorité (réversible). repointTo = clé d\'un AUTRE niveau vivant vers laquelle repointer toutes les cartes/tâches récurrentes/modèles qui l\'utilisaient ; absent = elles perdent leur priorité (jamais rejetées).',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/priorities/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Repoint To',
							name: 'repointTo',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'deleteTaskProperty',
			name: 'Delete Task Property',
			action: 'Supprime une propriété custom (réversible 30 jours)',
			description: 'Supprime une propriété custom (réversible 30 jours). Les valeurs en cellule sont conservées.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/property-definitions/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'deleteTaskStatus',
			name: 'Delete Task Status',
			action: 'Supprime un statut (réversible)',
			description: 'Supprime un statut (réversible). repointTo = clé d\'un AUTRE statut vivant vers laquelle repointer les colonnes qui le portaient ; absent = elles retombent sur leur name/status_category legacy (jamais de perte). Refusé sur le dernier statut vivant du projet.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/statuses/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Repoint To',
							name: 'repointTo',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'deleteTaskTemplate',
			name: 'Delete Task Template',
			action: 'Delete a task template',
			description: 'Delete a task template',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/templates/{template_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Template ID',
					name: 'template_id',
					type: 'string',
					required: true,
					description: 'The template ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'executeTaskWithAgent',
			name: 'Execute Task With Agent',
			action: 'Run the AI collaborator on a task card: mode "chat" (actionable advice, 1 credit) or "deliverable" (first draft attached to the task, 3 credits)',
			description: 'Run the AI collaborator on a task card: mode "chat" (actionable advice, 1 credit) or "deliverable" (first draft attached to the task, 3 credits). Paid AI action — approval-gated.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/agent/execute","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Mode',
					name: 'mode',
					type: 'options',
					required: true,
					default: 'chat',
					options: [
						{ name: 'Chat', value: 'chat' },
						{ name: 'Deliverable', value: 'deliverable' },
					],
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Agent ID',
							name: 'agentId',
							type: 'string',
							description: 'Optional persona collaborator ID',
							default: '',
						},
						{
							displayName: 'Instructions',
							name: 'instructions',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'generateDueRecurringTasks',
			name: 'Generate Due Recurring Tasks',
			action: 'Materialise, right now, the occurrences that are DUE (next_due_date ≤ today) of the active recurring tasks of a board — one new card per due definition — and advance their next date',
			description: 'Materialise, right now, the occurrences that are DUE (next_due_date ≤ today) of the active recurring tasks of a board — one new card per due definition — and advance their next date. Only the recurring definitions CREATED by the person are considered (a colleague\'s recurrences on the same board are not generated). Returns `{ generatedCount }`; calling again the same day creates nothing more. Use it when the person says « génère mes tâches récurrentes » or when `list_board_recurring_tasks` shows overdue `next_due_date`s. Nothing to do with `create_recurring_task`, which only defines a recurrence.',
			routeSpec: {"method":"POST","path":"/api/tasks/recurring/generate","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'boardId',
					type: 'string',
					required: true,
					description: 'Board ID (from list_boards)',
					default: '',
				}
			],
		},
		{
			value: 'getBoard',
			name: 'Get Board',
			action: 'Board details: columns and the board\'s cards (compact)',
			description: 'Board details: columns and the board\'s cards (compact). Use it to READ a board, not to create a card.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards/{board_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'getBoardSchema',
			name: 'Get Board Schema',
			action: 'Structure d\'un board kanban AVANT d\'écrire : colonnes (catégories de statut) et tags projet',
			description: 'Structure d\'un board kanban AVANT d\'écrire : colonnes (catégories de statut) et tags projet. Les propriétés custom valides vivent dans task_property_definitions — appeler list_task_properties pour les connaître.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards/{board_id}/schema","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'getCardActivity',
			name: 'Get Card Activity',
			action: 'Activity on a card',
			description: 'Activity on a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/activity","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'getCardProperties',
			name: 'Get Card Properties',
			action: 'Read custom properties on a card',
			description: 'Read custom properties on a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/properties","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'getTasksDashboard',
			name: 'Get Tasks Dashboard',
			action: 'Tasks dashboard: active, overdue, completed',
			description: 'Tasks dashboard: active, overdue, completed',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/dashboard","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Project ID',
							name: 'project_id',
							type: 'string',
							description: 'Project ID (optional)',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'instantiateCardTemplate',
			name: 'Instantiate Card Template',
			action: 'Crée une vraie tâche (carte kanban) sur un board à partir d’un modèle',
			description: 'Crée une vraie tâche (carte kanban) sur un board à partir d’un modèle. Le modèle DOIT appartenir au même projet que le board cible.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/task-templates/{id}/instantiate","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Board ID',
					name: 'boardId',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'linkCardToActionPlanTask',
			name: 'Link Card To Action Plan Task',
			action: 'Link a Kanban card to an action-plan task (gives transitive access to jalon + phase via the tache)',
			description: 'Link a Kanban card to an action-plan task (gives transitive access to jalon + phase via the tache)',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/action-plan-link","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Tache ID',
					name: 'tache_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'linkCardToGithubIssue',
			name: 'Link Card To Github Issue',
			action: 'Link a card to a GitHub issue/PR',
			description: 'Link a card to a GitHub issue/PR',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/{card_id}/github-links","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Issue URL',
					name: 'issue_url',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Repo ID',
							name: 'repo_id',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'linkCardToProject',
			name: 'Link Card To Project',
			action: 'Move a card to a board belonging to ANOTHER project: the card changes column, and therefore board and project',
			description: 'Move a card to a board belonging to ANOTHER project: the card changes column, and therefore board and project. Call list_boards on the destination project to get the target column, and say where the card lands in it — both fields are required by the route, there is no "append at the end" default.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/move","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'UUID of the card to move',
					default: '',
				},
				{
					displayName: 'Column ID',
					name: 'columnId',
					type: 'string',
					required: true,
					description: 'UUID of the DESTINATION column, from list_boards on the target project (columns[].ID)',
					default: '',
				},
				{
					displayName: 'Position',
					name: 'position',
					type: 'number',
					required: true,
					description: '0-based rank the card takes inside the destination column: 0 puts it on top, N puts it after the N cards already there. When the person did not say, send 0 — a rank beyond the column length simply lands last.',
					default: 0,
				}
			],
		},
		{
			value: 'linkIssueToTask',
			name: 'Link Issue To Task',
			action: 'Link a GitHub issue, PR, branch, or commit to a kanban card',
			description: 'Link a GitHub issue, PR, branch, or commit to a kanban card',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/{card_id}/github-links","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Github Repo ID',
					name: 'github_repo_id',
					type: 'string',
					required: true,
					description: 'Project_github_repos.ID (UUID)',
					default: '',
				},
				{
					displayName: 'Link Type',
					name: 'link_type',
					type: 'options',
					required: true,
					default: 'branch',
					options: [
						{ name: 'Branch', value: 'branch' },
						{ name: 'Commit', value: 'commit' },
						{ name: 'Issue', value: 'issue' },
						{ name: 'Pr', value: 'pr' },
					],
				},
				{
					displayName: 'External ID',
					name: 'external_id',
					type: 'string',
					required: true,
					description: 'Issue/PR number, branch name, or commit SHA',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'State',
							name: 'state',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
						{
							displayName: 'URL',
							name: 'url',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'linkTaskMeeting',
			name: 'Link Task Meeting',
			action: 'Link an existing calendar meeting (create it first with create_meeting) to a kanban card',
			description: 'Link an existing calendar meeting (create it first with create_meeting) to a kanban card',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/meetings","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Event ID',
					name: 'event_id',
					type: 'string',
					required: true,
					description: 'UUID of an existing meeting/calendar event',
					default: '',
				}
			],
		},
		{
			value: 'listBoardRecurringTasks',
			name: 'List Board Recurring Tasks',
			action: 'Recurring task definitions of a board',
			description: 'Recurring task definitions of a board',
			routeSpec: {"method":"GET","path":"/api/tasks/boards/{board_id}/recurring","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listBoards',
			name: 'List Boards',
			action: 'List the project\'s kanban boards',
			description: 'List the project\'s kanban boards. Each board carries its card count and its default column (ID, name); the full column list is included only when the project has few boards. One call is enough to pick a destination for create_card.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Project ID',
							name: 'project_id',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'listCardActivity',
			name: 'List Card Activity',
			action: 'Activity log of a card (moves, comments, edits)',
			description: 'Activity log of a card (moves, comments, edits)',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/activity","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardAgentRuns',
			name: 'List Card Agent Runs',
			action: 'History of AI collaborator runs on a task card (last 20)',
			description: 'History of AI collaborator runs on a task card (last 20)',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/agent/runs","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardAttachments',
			name: 'List Card Attachments',
			action: 'Attachments on a card',
			description: 'Attachments on a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/attachments","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardChecklists',
			name: 'List Card Checklists',
			action: 'Checklists attached to a card',
			description: 'Checklists attached to a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/checklists","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardComments',
			name: 'List Card Comments',
			action: 'Comments on a card',
			description: 'Comments on a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/comments","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardDependencies',
			name: 'List Card Dependencies',
			action: 'Dependencies of a card in both directions (blockedBy / blocks) + isBlocked flag',
			description: 'Dependencies of a card in both directions (blockedBy / blocks) + isBlocked flag',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/dependencies","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardGithubLinks',
			name: 'List Card Github Links',
			action: 'GitHub issue/PR links attached to a card',
			description: 'GitHub issue/PR links attached to a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/{card_id}/github-links","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardSubtasks',
			name: 'List Card Subtasks',
			action: 'Subtasks of a card (child cards)',
			description: 'Subtasks of a card (child cards)',
			routeSpec: {"method":"GET","path":"/api/tasks/cards/{card_id}/subtasks","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listCardTemplates',
			name: 'List Card Templates',
			action: 'Liste les modèles de tâche vivants d\'un projet — à appeler AVANT instantiate_card_template pour connaître les IDs/noms disponibles',
			description: 'Liste les modèles de tâche vivants d\'un projet — à appeler AVANT instantiate_card_template pour connaître les IDs/noms disponibles. Distinct de list_task_templates (modèles de BOARD entier).',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/task-templates","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listChecklists',
			name: 'List Checklists',
			action: 'Checklists on a card',
			description: 'Checklists on a card',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/cards/{card_id}/checklists","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listProjectTaskTags',
			name: 'List Project Task Tags',
			action: 'Task tags of a project',
			description: 'Task tags of a project',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/tags","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listSprints',
			name: 'List Sprints',
			action: 'List a board\'s milestones',
			description: 'List a board\'s milestones. The product calls these MILESTONES (« jalons » in French) since 2026-08-08 — say « milestone/jalon » to the user, never « sprint ». The table and the `sprint_ids` field keep their historical names.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/sprints","queryParams":["board_id:boardId"]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listTaskBoardComments',
			name: 'List Task Board Comments',
			action: 'Comments on a task project (board), Informations tab',
			description: 'Comments on a task project (board), Informations tab',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards/{board_id}/comments","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listTaskBoardLinks',
			name: 'List Task Board Links',
			action: 'Resource links attached to a task project (board): notes, drive files, external URLs, meetings',
			description: 'Resource links attached to a task project (board): notes, drive files, external URLs, meetings',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards/{board_id}/links","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listTaskBoardMeetings',
			name: 'List Task Board Meetings',
			action: 'Meetings linked to a task project (board): the board meeting entities (public.meetings, with status and summary) plus any bare linked calendar events not yet backed by a meeting',
			description: 'Meetings linked to a task project (board): the board meeting entities (public.meetings, with status and summary) plus any bare linked calendar events not yet backed by a meeting',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/boards/{board_id}/meetings","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'listTaskDomains',
			name: 'List Task Domains',
			action: 'Liste les domaines d\'activité d\'un projet (référentiel du Gantt de portefeuille), triés par position — crée « Non classé » au passage s\'il n\'existe pas encore',
			description: 'Liste les domaines d\'activité d\'un projet (référentiel du Gantt de portefeuille), triés par position — crée « Non classé » au passage s\'il n\'existe pas encore. À appeler AVANT de poser un ID de domaine sur un board.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/domains","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listTaskPriorities',
			name: 'List Task Priorities',
			action: 'Liste les DÉFINITIONS des niveaux de priorité configurés sur un projet (clés/libellés/rang, ex',
			description: 'Liste les DÉFINITIONS des niveaux de priorité configurés sur un projet (clés/libellés/rang, ex. low/medium/high/urgent), triées du plus urgent au moins urgent (rank DESC) — PAS une liste de tâches, PAS un outil de consultation de to-do. À appeler AVANT de poser priority sur une carte/tâche récurrente/modèle, pour connaître les clés valides. Pour afficher les tâches de l\'utilisateur, utiliser list_cards.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/priorities","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listTaskProperties',
			name: 'List Task Properties',
			action: 'Liste les propriétés custom des tâches d\'un projet (portée projet + board)',
			description: 'Liste les propriétés custom des tâches d\'un projet (portée projet + board). À appeler AVANT set_card_property pour connaître les clés, types et choix valides.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/property-definitions","queryParams":["project_id:projectId","board_id:boardId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Board ID',
							name: 'board_id',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'listTaskStatuses',
			name: 'List Task Statuses',
			action: 'Liste les statuts vivants d\'un projet, triés du moins avancé au plus avancé (rank ASC — backlog en premier)',
			description: 'Liste les statuts vivants d\'un projet, triés du moins avancé au plus avancé (rank ASC — backlog en premier). À appeler AVANT de poser category/color sur create_task_status/update_task_status.',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/statuses","queryParams":["project_id:projectId"]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'project_id',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'listTaskTemplates',
			name: 'List Task Templates',
			action: 'Task templates',
			description: 'Task templates',
			routeSpec: {"method":"GET","path":"/api/aurentia/tasks/templates","queryParams":[]},
			properties: [

			],
		},
		{
			value: 'moveCard',
			name: 'Move Card',
			action: 'Move a card',
			description: 'Move a card',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/move","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Column ID',
					name: 'columnId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Position',
							name: 'position',
							type: 'number',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'placeStatusOnBoard',
			name: 'Place Status On Board',
			action: 'Show a project-level task status on ONE board: a column carrying that status is created on the board (returns `{ columnId }`)',
			description: 'Show a project-level task status on ONE board: a column carrying that status is created on the board (returns `{ columnId }`). Statuses live at project level (`list_task_statuses`, `create_task_status`); a board only displays a subset of them, and this is how you add one to a board. `position` is the column index (0 = leftmost); omit it to append on the right. The status must belong to the board\'s project and be alive (404 otherwise). To add a column WITHOUT a project status (a purely local column) use `create_column` instead. Read the board first with `get_board_schema` so you do not place a status that is already there.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/{boardId}/statuses","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'boardId',
					type: 'string',
					required: true,
					description: 'Board ID (from list_boards)',
					default: '',
				},
				{
					displayName: 'Status ID',
					name: 'statusId',
					type: 'string',
					required: true,
					description: 'Project task status ID (from list_task_statuses)',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Position',
							name: 'position',
							type: 'number',
							description: 'Column index. Omit to append.',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'postponeRecurringTask',
			name: 'Postpone Recurring Task',
			action: 'Postpone the next occurrence of a recurring task by N days (1-90)',
			description: 'Postpone the next occurrence of a recurring task by N days (1-90)',
			routeSpec: {"method":"POST","path":"/api/tasks/recurring/{recurring_id}/postpone","queryParams":[]},
			properties: [
				{
					displayName: 'Recurring ID',
					name: 'recurring_id',
					type: 'string',
					required: true,
					description: 'The recurring ID for this operation',
					default: '',
				},
				{
					displayName: 'Days',
					name: 'days',
					type: 'number',
					required: true,
					description: 'Integer 1-90',
					default: 0,
				}
			],
		},
		{
			value: 'removeCardDependency',
			name: 'Remove Card Dependency',
			action: 'Remove a dependency between two cards',
			description: 'Remove a dependency between two cards',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/dependencies/{dependency_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Dependency ID',
					name: 'dependency_id',
					type: 'string',
					required: true,
					description: 'The dependency ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'removeStatusFromBoard',
			name: 'Remove Status From Board',
			action: 'Take a status off ONE board: the column carrying it is DELETED from that board',
			description: 'Take a status off ONE board: the column carrying it is DELETED from that board. The project status itself survives (other boards keep it; `delete_task_status` is the tool that removes a status everywhere). If the column still holds cards you MUST say where they go with `moveCardsTo` (the ID of ANOTHER column of the same board, from `get_board_schema`) — without it the server refuses with 400 rather than deleting cards silently. The last column of a board cannot be removed (400). Cards moved this way change status in one go: tell the person how many cards move, and where.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/boards/{boardId}/statuses/{statusId}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'boardId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Status ID',
					name: 'statusId',
					type: 'string',
					required: true,
					description: 'Project task status ID currently placed on the board',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Move Cards To',
							name: 'moveCardsTo',
							type: 'string',
							description: 'Target column ID on the SAME board for the cards of the removed column. Required when the column is not empty.',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'reorderCards',
			name: 'Reorder Cards',
			action: 'Reorder the cards INSIDE one kanban column, top to bottom',
			description: 'Reorder the cards INSIDE one kanban column, top to bottom. Both fields are required and they go together: the column being reordered, and the full list of the cards it holds in the wanted order. To move a card to ANOTHER column, use move_card / link_card_to_project instead — this tool never changes which column a card belongs to.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Column ID',
					name: 'columnId',
					type: 'string',
					required: true,
					description: 'UUID of the column whose cards are being reordered (list_boards returns columns[].ID)',
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'EVERY card UUID of that column, in the wanted order, topmost first. A partial list leaves the omitted cards at stale positions. (provide a JSON array)',
					default: '[]',
				}
			],
		},
		{
			value: 'reorderChecklists',
			name: 'Reorder Checklists',
			action: 'Reorder the checklist items of a card',
			description: 'Reorder the checklist items of a card. Read the card first (get_card) to collect its checklist item IDs: you send back ALL of them in the wanted order, not just the one that moved.',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/cards/{card_id}/checklists/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'Card UUID that owns the checklist',
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'EVERY checklist item UUID of that card, in the wanted order, first item first. An ID missing from the list keeps its old position. (provide a JSON array)',
					default: '[]',
				}
			],
		},
		{
			value: 'reorderColumns',
			name: 'Reorder Columns',
			action: 'Reorder the columns of a kanban board, left to right',
			description: 'Reorder the columns of a kanban board, left to right. Read the board with list_boards first: you must send back ALL of its column IDs, in the order you want them displayed — this is a full reordering, not a move of one column. A column left out of the list keeps its old rank and the board ends up in an order nobody asked for.',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/boards/{board_id}/columns/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'Board UUID (list_boards)',
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'EVERY column UUID of that board, in the wanted display order, leftmost first. Take them from list_boards (columns[].ID) — never invent one, and never send a partial list. (provide a JSON array)',
					default: '[]',
				}
			],
		},
		{
			value: 'reorderTaskPriorities',
			name: 'Reorder Task Priorities',
			action: 'Réordonne l\'INTÉGRALITÉ des niveaux de priorité vivants d\'un projet (IDs doit lister chaque ID vivant exactement une fois)',
			description: 'Réordonne l\'INTÉGRALITÉ des niveaux de priorité vivants d\'un projet (IDs doit lister chaque ID vivant exactement une fois). IDs[0] = le plus urgent.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/priorities/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'Provide a JSON array',
					default: '[]',
				}
			],
		},
		{
			value: 'reorderTaskProperties',
			name: 'Reorder Task Properties',
			action: 'Réordonne les propriétés custom d\'un projet (portées projet et board mélangées)',
			description: 'Réordonne les propriétés custom d\'un projet (portées projet et board mélangées)',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/property-definitions/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					description: 'Project UUID — porte la portée, vérifiée côté serveur',
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'IDs des définitions dans le nouvel ordre. Champs camelCase (projectId/IDs) : le corps de la route les exige tels quels. (provide a JSON array)',
					default: '[]',
				}
			],
		},
		{
			value: 'reorderTaskStatuses',
			name: 'Reorder Task Statuses',
			action: 'Réordonne l\'INTÉGRALITÉ des statuts vivants d\'un projet (IDs doit lister chaque ID vivant exactement une fois)',
			description: 'Réordonne l\'INTÉGRALITÉ des statuts vivants d\'un projet (IDs doit lister chaque ID vivant exactement une fois). IDs[0] = le MOINS avancé (début de flux) — inverse de reorder_task_priorities, où IDs[0] est le plus urgent.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/statuses/reorder","queryParams":[]},
			properties: [
				{
					displayName: 'Project ID',
					name: 'projectId',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'IDs',
					name: 'ids',
					type: 'json',
					required: true,
					description: 'Provide a JSON array',
					default: '[]',
				}
			],
		},
		{
			value: 'repointTaskPropertyChoice',
			name: 'Repoint Task Property Choice',
			action: 'Rewrite EVERY task cell that holds one choice of a select / multi-select property so it holds another choice instead (merging two duplicate options, or emptying a choice before removing it)',
			description: 'Rewrite EVERY task cell that holds one choice of a select / multi-select property so it holds another choice instead (merging two duplicate options, or emptying a choice before removing it). `fromChoiceId` and `toChoiceId` are choice IDs as listed by `list_task_properties` on that property; `toChoiceId: null` CLEARS the cells instead of repointing them. This touches all matching cards of the project at once and cannot be undone by a single call — say how many cards are concerned (the result gives `updated`) and confirm before. The choice definitions themselves are not changed here: edit them with `update_task_property` afterwards.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/property-definitions/{id}/repoint-choice","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'Property definition ID (from list_task_properties)',
					default: '',
				},
				{
					displayName: 'From Choice ID',
					name: 'fromChoiceId',
					type: 'string',
					required: true,
					description: 'Choice whose cells are rewritten',
					default: '',
				},
				{
					displayName: 'To Choice ID',
					name: 'toChoiceId',
					type: 'string',
					required: true,
					description: 'Choice that replaces it. null clears the cells.',
					default: '',
				}
			],
		},
		{
			value: 'restoreCardTemplate',
			name: 'Restore Card Template',
			action: 'Undo `delete_card_template`: a task template that was soft-deleted (reversible for 30 days) comes back exactly as it was',
			description: 'Undo `delete_card_template`: a task template that was soft-deleted (reversible for 30 days) comes back exactly as it was. `list_card_templates` hides deleted templates, so the ID must come from the deletion itself (the person just deleted it in this conversation, or knows its ID). Mirrors `restore_task_status` and `restore_task_property`. 404 if the template does not exist or is not the person\'s.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/task-templates/{id}/restore","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'Task template ID (the one that was deleted)',
					default: '',
				}
			],
		},
		{
			value: 'restoreTaskPriority',
			name: 'Restore Task Priority',
			action: 'Restaure un niveau de priorité supprimé',
			description: 'Restaure un niveau de priorité supprimé',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/priorities/{id}/restore","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'restoreTaskProperty',
			name: 'Restore Task Property',
			action: 'Restaure une propriété custom supprimée (soft delete, réversible 30 jours)',
			description: 'Restaure une propriété custom supprimée (soft delete, réversible 30 jours). Les valeurs de cellule déjà écrites sont rebranchées telles quelles.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/property-definitions/{id}/restore","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'restoreTaskStatus',
			name: 'Restore Task Status',
			action: 'Restaure un statut supprimé',
			description: 'Restaure un statut supprimé',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/statuses/{id}/restore","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'setCardOwners',
			name: 'Set Card Owners',
			action: 'Deprecated alias — writes assigned_to (the canonical multi-assignee field)',
			description: 'Deprecated alias — writes assigned_to (the canonical multi-assignee field). The owner_user_ids input is mapped server-side to assigned_to; prefer update_card with assigned_to.',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/cards/{card_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Owner User IDs',
					name: 'owner_user_ids',
					type: 'json',
					required: true,
					description: 'Provide a JSON array',
					default: '[]',
				}
			],
		},
		{
			value: 'setCardProperty',
			name: 'Set Card Property',
			action: 'Set a single custom property on a card',
			description: 'Set a single custom property on a card',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/cards/{card_id}/properties","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Key',
					name: 'key',
					type: 'string',
					required: true,
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Value',
							name: 'value',
							type: 'json',
							description: 'Provide a JSON object',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'setCardStatus',
			name: 'Set Card Status',
			action: 'Set status of a card (todo|in_progress|completed)',
			description: 'Set status of a card (todo|in_progress|completed)',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/cards/{card_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Status',
					name: 'status',
					type: 'options',
					required: true,
					default: 'completed',
					options: [
						{ name: 'Completed', value: 'completed' },
						{ name: 'In Progress', value: 'in_progress' },
						{ name: 'Todo', value: 'todo' },
					],
				}
			],
		},
		{
			value: 'shiftBoardDates',
			name: 'Shift Board Dates',
			action: 'Move the start AND end dates of several task boards (chantiers/projets du portefeuille) by the same number of days at once — « décale tous mes chantiers d\'une semaine », « avance ces trois projets de 3 jours »',
			description: 'Move the start AND end dates of several task boards (chantiers/projets du portefeuille) by the same number of days at once — « décale tous mes chantiers d\'une semaine », « avance ces trois projets de 3 jours ». `boardIds` (1-200, from `list_boards`) and `shiftDays` (integer, positive = later, negative = earlier, never 0). Boards without dates are refused by name rather than silently skipped, so send only boards that carry dates. Bulk and immediate: read back which boards and how many days before calling; undo = call again with the opposite number. For ONE board\'s dates use `update_board`.',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/boards/shift","queryParams":[]},
			properties: [
				{
					displayName: 'Board IDs',
					name: 'boardIds',
					type: 'json',
					required: true,
					description: 'Provide a JSON array',
					default: '[]',
				},
				{
					displayName: 'Shift Days',
					name: 'shiftDays',
					type: 'number',
					required: true,
					description: 'Days to shift; negative moves earlier; 0 is refused',
					default: 0,
				}
			],
		},
		{
			value: 'skipRecurringTask',
			name: 'Skip Recurring Task',
			action: 'Skip the next occurrence of a recurring task (recurrence stays active)',
			description: 'Skip the next occurrence of a recurring task (recurrence stays active)',
			routeSpec: {"method":"POST","path":"/api/tasks/recurring/{recurring_id}/skip","queryParams":[]},
			properties: [
				{
					displayName: 'Recurring ID',
					name: 'recurring_id',
					type: 'string',
					required: true,
					description: 'The recurring ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'toggleCardSubtask',
			name: 'Toggle Card Subtask',
			action: 'Toggle a subtask between completed and todo',
			description: 'Toggle a subtask between completed and todo',
			routeSpec: {"method":"POST","path":"/api/tasks/subtasks/{subtask_id}/toggle","queryParams":[]},
			properties: [
				{
					displayName: 'Subtask ID',
					name: 'subtask_id',
					type: 'string',
					required: true,
					description: 'The subtask ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'toggleChecklistItem',
			name: 'Toggle Checklist Item',
			action: 'Check/uncheck a checklist item',
			description: 'Check/uncheck a checklist item',
			routeSpec: {"method":"POST","path":"/api/aurentia/tasks/cards/{card_id}/checklists/{item_id}/toggle","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Item ID',
					name: 'item_id',
					type: 'string',
					required: true,
					description: 'The item ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'unlinkCardFromActionPlan',
			name: 'Unlink Card From Action Plan',
			action: 'Remove the link between a Kanban card and its action-plan task',
			description: 'Remove the link between a Kanban card and its action-plan task. Idempotent.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/action-plan-link","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'unlinkCardGithubLink',
			name: 'Unlink Card Github Link',
			action: 'Detach ONE GitHub issue / PR / branch / commit link from a kanban card',
			description: 'Detach ONE GitHub issue / PR / branch / commit link from a kanban card. Only the link row goes — the card and the GitHub object are untouched, nothing is posted to GitHub. `linkId` is the link\'s own ID from `list_card_github_links` (not the issue number). Only the person who created the link may remove it (403 for anyone else, even the card owner). Re-attach later with `link_card_to_github_issue`.',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/{cardId}/github-links/{linkId}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'cardId',
					type: 'string',
					required: true,
					description: 'Kanban card ID',
					default: '',
				},
				{
					displayName: 'Link ID',
					name: 'linkId',
					type: 'string',
					required: true,
					description: 'GitHub link ID (from list_card_github_links)',
					default: '',
				}
			],
		},
		{
			value: 'unlinkTaskMeeting',
			name: 'Unlink Task Meeting',
			action: 'Unlink a meeting from a kanban card (does not delete the meeting itself)',
			description: 'Unlink a meeting from a kanban card (does not delete the meeting itself)',
			routeSpec: {"method":"DELETE","path":"/api/aurentia/tasks/cards/{card_id}/meetings/{event_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Event ID',
					name: 'event_id',
					type: 'string',
					required: true,
					description: 'The event ID for this operation',
					default: '',
				}
			],
		},
		{
			value: 'updateBoard',
			name: 'Update Board',
			action: 'Update a board (project)',
			description: 'Update a board (project). Only provide the fields to change.',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/boards/{board_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Board ID',
					name: 'board_id',
					type: 'string',
					required: true,
					description: 'The board ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'End Date',
							name: 'end_date',
							type: 'string',
							description: 'ISO date (YYYY-MM-DD)',
							default: '',
						},
						{
							displayName: 'Is Priority',
							name: 'is_priority',
							type: 'boolean',
							description: 'Whether to enable is priority',
							default: false,
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Owner ID',
							name: 'owner_id',
							type: 'string',
							description: 'User UUID of the project owner (responsable). MUST be a member of the project — the server rejects anyone else. Pass null to unassign.',
							default: '',
						},
						{
							displayName: 'Start Date',
							name: 'start_date',
							type: 'string',
							description: 'ISO date (YYYY-MM-DD)',
							default: '',
						},
						{
							displayName: 'Status',
							name: 'status',
							type: 'string',
							description: 'One of \'a_venir\' | \'en_cours\' | \'en_pause\' | \'termine\' | \'bloque\' | \'archive\'',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateCardComment',
			name: 'Update Card Comment',
			action: 'Update a comment',
			description: 'Update a comment',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/cards/{card_id}/comments/{comment_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Comment ID',
					name: 'comment_id',
					type: 'string',
					required: true,
					description: 'The comment ID for this operation',
					default: '',
				},
				{
					displayName: 'Content',
					name: 'content',
					type: 'string',
					required: true,
					default: '',
				}
			],
		},
		{
			value: 'updateCardSubtask',
			name: 'Update Card Subtask',
			action: 'Update a subtask (title, status, priority, due date…)',
			description: 'Update a subtask (title, status, priority, due date…)',
			routeSpec: {"method":"PATCH","path":"/api/tasks/subtasks/{subtask_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Subtask ID',
					name: 'subtask_id',
					type: 'string',
					required: true,
					description: 'The subtask ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Assigned To',
							name: 'assigned_to',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Due Date',
							name: 'due_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimated_minutes',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
						{
							displayName: 'Status',
							name: 'status',
							type: 'options',
							default: 'completed',
							options: [
								{ name: 'Completed', value: 'completed' },
								{ name: 'In Progress', value: 'in_progress' },
								{ name: 'Todo', value: 'todo' },
							],
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateCardTemplate',
			name: 'Update Card Template',
			action: 'Met à jour un modèle de tâche (patch partiel — seules les clés fournies changent)',
			description: 'Met à jour un modèle de tâche (patch partiel — seules les clés fournies changent)',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/task-templates/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Assigned To',
							name: 'assignedTo',
							type: 'json',
							description: 'User UUIDs. Absent/empty = the template assigns no one. (provide a JSON array)',
							default: '[]',
						},
						{
							displayName: 'Checklist Items',
							name: 'checklistItems',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Custom Properties',
							name: 'customProperties',
							type: 'json',
							description: 'Provide a JSON object',
							default: '{}',
						},
						{
							displayName: 'Description Md',
							name: 'descriptionMd',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Due Offset Days',
							name: 'dueOffsetDays',
							type: 'number',
							description: 'Signed day offset from the instantiation date (e.g. -2 = two days before a milestone). Resolved to an absolute date only when instantiate_card_template runs — never store an absolute date on a template.',
							default: 0,
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimatedMinutes',
							type: 'number',
							description: 'Estimate in minutes (1-43200). Absent = the template poses no estimate.',
							default: 0,
						},
						{
							displayName: 'Is Code Task',
							name: 'isCodeTask',
							type: 'boolean',
							description: 'Whether to enable is code task',
							default: false,
						},
						{
							displayName: 'Is Important',
							name: 'isImportant',
							type: 'boolean',
							description: 'Whether to enable is important',
							default: false,
						},
						{
							displayName: 'Is Milestone',
							name: 'isMilestone',
							type: 'boolean',
							description: 'Whether to enable is milestone',
							default: false,
						},
						{
							displayName: 'Is Urgent',
							name: 'isUrgent',
							type: 'boolean',
							description: 'Whether to enable is urgent',
							default: false,
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
						{
							displayName: 'Start Offset Days',
							name: 'startOffsetDays',
							type: 'number',
							description: 'Signed day offset from the instantiation date (e.g. -2 = two days before a milestone). Resolved to an absolute date only when instantiate_card_template runs — never store an absolute date on a template.',
							default: 0,
						},
						{
							displayName: 'Status Category',
							name: 'statusCategory',
							type: 'options',
							default: 'backlog',
							options: [
								{ name: 'Backlog', value: 'backlog' },
								{ name: 'Blocked', value: 'blocked' },
								{ name: 'Done', value: 'done' },
								{ name: 'In Progress', value: 'in_progress' },
								{ name: 'Todo', value: 'todo' },
							],
						},
						{
							displayName: 'Tag IDs',
							name: 'tagIds',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateChecklistItem',
			name: 'Update Checklist Item',
			action: 'Update a checklist item — rename it and/or check it off',
			description: 'Update a checklist item — rename it and/or check it off',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/cards/{card_id}/checklists/{item_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Card ID',
					name: 'card_id',
					type: 'string',
					required: true,
					description: 'The card ID for this operation',
					default: '',
				},
				{
					displayName: 'Item ID',
					name: 'item_id',
					type: 'string',
					required: true,
					description: 'The item ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Is Completed',
							name: 'is_completed',
							type: 'boolean',
							description: 'Whether true checks the item, false unchecks it. Prefer this over toggle_checklist_item when you know the target state.',
							default: false,
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateColumn',
			name: 'Update Column',
			action: 'Update a column',
			description: 'Update a column',
			routeSpec: {"method":"PUT","path":"/api/aurentia/tasks/columns/{column_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Column ID',
					name: 'column_id',
					type: 'string',
					required: true,
					description: 'The column ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateProjectTaskTag',
			name: 'Update Project Task Tag',
			action: 'Rename or recolor a project task tag',
			description: 'Rename or recolor a project task tag',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/tags/{tag_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Tag ID',
					name: 'tag_id',
					type: 'string',
					required: true,
					description: 'The tag ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							default: '',
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateRecurringTask',
			name: 'Update Recurring Task',
			action: 'Update a recurring task definition (frequency, title, active…)',
			description: 'Update a recurring task definition (frequency, title, active…)',
			routeSpec: {"method":"PATCH","path":"/api/tasks/recurring/{recurring_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Recurring ID',
					name: 'recurring_id',
					type: 'string',
					required: true,
					description: 'The recurring ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Day Of Month',
							name: 'day_of_month',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Day Of Week',
							name: 'day_of_week',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Description',
							name: 'description',
							type: 'string',
							default: '',
						},
						{
							displayName: 'End Date',
							name: 'end_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Estimated Minutes',
							name: 'estimated_minutes',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Frequency',
							name: 'frequency',
							type: 'options',
							default: 'bi-weekly',
							options: [
								{ name: 'Bi Weekly', value: 'bi-weekly' },
								{ name: 'Daily', value: 'daily' },
								{ name: 'Every N Days', value: 'every_n_days' },
								{ name: 'Every N Weeks', value: 'every_n_weeks' },
								{ name: 'Monthly', value: 'monthly' },
								{ name: 'Weekly', value: 'weekly' },
								{ name: 'Yearly', value: 'yearly' },
							],
						},
						{
							displayName: 'Is Active',
							name: 'is_active',
							type: 'boolean',
							description: 'Whether to enable is active',
							default: false,
						},
						{
							displayName: 'Month Of Year',
							name: 'month_of_year',
							type: 'number',
							default: 0,
						},
						{
							displayName: 'Next Due Date',
							name: 'next_due_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Priority',
							name: 'priority',
							type: 'string',
							description: 'Priority key (project-scoped, free list) — call list_task_priorities to discover valid keys/labels/ranks. Default-seeded projects have \'low\'/\'medium\'/\'high\'/\'urgent\'.',
							default: '',
						},
						{
							displayName: 'Start Date',
							name: 'start_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Title',
							name: 'title',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateSprint',
			name: 'Update Sprint',
			action: 'Update a milestone (« jalon »)',
			description: 'Update a milestone (« jalon »)',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/sprints/{sprint_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Sprint ID',
					name: 'sprint_id',
					type: 'string',
					required: true,
					description: 'The sprint ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							default: '',
						},
						{
							displayName: 'End Date',
							name: 'end_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Goal',
							name: 'goal',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Start Date',
							name: 'start_date',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Status',
							name: 'status',
							type: 'options',
							default: 'active',
							options: [
								{ name: 'Active', value: 'active' },
								{ name: 'Archived', value: 'archived' },
								{ name: 'Completed', value: 'completed' },
								{ name: 'Upcoming', value: 'upcoming' },
							],
						},
					],
				}
			],
		},
		{
			value: 'updateTaskDomain',
			name: 'Update Task Domain',
			action: 'Renomme/recolore/repositionne un domaine d\'activité',
			description: 'Renomme/recolore/repositionne un domaine d\'activité. Le domaine « Non classé » ne peut jamais être renommé (position/couleur restent modifiables).',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/domains/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							description: 'Clé de palette — cf. list_task_domains.',
							default: '',
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Position',
							name: 'position',
							type: 'number',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'updateTaskPriority',
			name: 'Update Task Priority',
			action: 'Renomme/recolore/repositionne un niveau de priorité, ou le pose comme défaut du projet',
			description: 'Renomme/recolore/repositionne un niveau de priorité, ou le pose comme défaut du projet. La clé (référencée par les cartes) ne change jamais.',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/priorities/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							default: '',
						},
						{
							displayName: 'Is Default',
							name: 'isDefault',
							type: 'boolean',
							description: 'Whether to enable is default',
							default: false,
						},
						{
							displayName: 'Label',
							name: 'label',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Rank',
							name: 'rank',
							type: 'number',
							default: 0,
						},
					],
				}
			],
		},
		{
			value: 'updateTaskProperty',
			name: 'Update Task Property',
			action: 'Renomme une propriété custom, change ses choix ou leurs couleurs',
			description: 'Renomme une propriété custom, change ses choix ou leurs couleurs. Le type se change via un appel dédié.',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/property-definitions/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Label',
							name: 'label',
							type: 'string',
							default: '',
						},
						{
							displayName: 'Options',
							name: 'options',
							type: 'json',
							description: 'Provide a JSON object',
							default: '{}',
						},
					],
				}
			],
		},
		{
			value: 'updateTaskStatus',
			name: 'Update Task Status',
			action: 'Renomme/recatégorise/recolore un statut, ou le pose comme défaut du projet',
			description: 'Renomme/recatégorise/recolore un statut, ou le pose comme défaut du projet. La clé (key, référencée par les colonnes) ne change jamais. Pas de rank ici : le rang ne se change que par reorder_task_statuses, seul à garantir un ordre total.',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/statuses/{id}","queryParams":[]},
			properties: [
				{
					displayName: 'ID',
					name: 'id',
					type: 'string',
					required: true,
					description: 'The ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Category',
							name: 'category',
							type: 'options',
							default: 'backlog',
							options: [
								{ name: 'Backlog', value: 'backlog' },
								{ name: 'Blocked', value: 'blocked' },
								{ name: 'Done', value: 'done' },
								{ name: 'In Progress', value: 'in_progress' },
								{ name: 'Todo', value: 'todo' },
							],
						},
						{
							displayName: 'Color',
							name: 'color',
							type: 'color',
							description: 'Clé de palette — cf. list_task_statuses. Jamais un hex.',
							default: '',
						},
						{
							displayName: 'Is Default',
							name: 'isDefault',
							type: 'boolean',
							description: 'Whether to enable is default',
							default: false,
						},
						{
							displayName: 'Label',
							name: 'label',
							type: 'string',
							default: '',
						},
					],
				}
			],
		},
		{
			value: 'updateTaskTemplate',
			name: 'Update Task Template',
			action: 'Update a task template (columns)',
			description: 'Update a task template (columns)',
			routeSpec: {"method":"PATCH","path":"/api/aurentia/tasks/templates/{template_id}","queryParams":[]},
			properties: [
				{
					displayName: 'Template ID',
					name: 'template_id',
					type: 'string',
					required: true,
					description: 'The template ID for this operation',
					default: '',
				},
				{
					displayName: 'Additional Fields',
					name: 'additionalFields',
					type: 'collection',
					placeholder: 'Add Field',
					default: {},
					options: [
						{
							displayName: 'Columns',
							name: 'columns',
							type: 'json',
							description: 'Provide a JSON array',
							default: '[]',
						},
						{
							displayName: 'Name',
							name: 'name',
							type: 'string',
							default: '',
						},
					],
				}
			],
		}
	],
};
